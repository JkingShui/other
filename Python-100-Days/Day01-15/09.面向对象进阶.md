...
'''
@property  的作用
在一个需要传入数据，并且这个数据有限制条件时使用
python没有类型检查，所以就需要get 和set方法
但是get 和 set 在使用上又很麻烦，就使用@property来简化操作
'''
...
[一篇关于slots的博客](https://www.cnblogs.com/rainfd/p/slots.html)
...
'''
普通方法就是对象能看见的
类方法（@classmethon）只有类能看到
静态方法（@staticmethod），所有对象公用
